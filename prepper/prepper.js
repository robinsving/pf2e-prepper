...
import { info, error } from "./utilities/Utilities.js";

// Create API for other modules to use
const API = {
  PrepperStorage: PrepperStorage,
  openSpellListsApp: (actor) => new PrepperApp(actor).render(true)
};
...
Hooks.on('renderActorSheet', (app, html, data) => {
  try {
    ...
    if (app.actor.type !== 'character' || !game.system.id.startsWith('pf2e')) return;
